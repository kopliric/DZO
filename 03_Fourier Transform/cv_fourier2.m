...
% The antialiasing filter. - a low-pass, namely a
% Gaussian filter with a spatial extent set to preserve the condition of
% the Sampling theorem. 
A = image_generator('Gaussian',size(x),size(x,1)/32);
...
S = image_generator('square',size(x),size(x,1)/8);
