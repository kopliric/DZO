...
%% 1b: Implemented simple kernels (average, sobel, gauss, dgauss, lgauss)
...
function kernel = gauss(ker_size, gauss_stddev)
    [x,y] = meshgrid(-floor(ker_size/2):floor(ker_size/2));
...
function kernel = lgauss(ker_size, gauss_stddev)
    [x,y] = meshgrid(-floor(ker_size/2):floor(ker_size/2));
    kernel = exp(-(x.^2 + y.^2) / (gauss_stddev^2));
    kernel = ((x.^2 + y.^2 - gauss_stddev^2) / gauss_stddev^4) .* kernel;
end

%% 3: Implemented simple separable kernels (average, sobel, gauss)
...
function [kernel_x, kernel_y] = gauss_sep(ker_size, gauss_stddev)
    x = -floor(ker_size/2):floor(ker_size/2);
    y = (-floor(ker_size/2):floor(ker_size/2))';

    kernel_x = exp(-x.^2 / gauss_stddev^2);
    kernel_x = kernel_x ./ sum(sum(kernel_x));

    kernel_y = exp(-y.^2 / gauss_stddev^2);
